    ...
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-8 pb-20 md:pb-0">
        {frames.map((frame, index) => {
          ...
          <div key={frame.id} className="flex flex-col gap-4 group">
            ...
            {/* 图片容器 */}
            <div 
              className={`relative aspect-[3/4] bg-[#0c0c0e] rounded-lg overflow-hidden border border-white/5 shadow-lg transition-all duration-500 ${
            ...
            >
              ...
              {frame.status === 'completed' && frame.imageUrl ? (
                ...
                <div className={`w-full h-full flex flex-col items-center justify-center p-6 text-center relative overflow-hidden bg-[#0c0c0e] ${frame.status === 'generating' ? 'bg-grain' : ''}`}>
                  
                  {/* GENERATING STATE ANIMATION */}
                  {frame.status === 'generating' && (
                    <>
                       <div className="absolute inset-0 bg-amber-900/5 mix-blend-overlay"></div>
                       <div className="absolute inset-0 animate-film-develop z-0 opacity-50 pointer-events-none"></div>
                       <div className="relative z-10 mb-6">
                           <div className="w-12 h-12 rounded-full border-[3px] border-zinc-800 border-t-amber-500/80 animate-spin"></div>
                           <div className="absolute inset-0 flex items-center justify-center">
                              <div className="w-1.5 h-1.5 bg-amber-500 rounded-full shadow-[0_0_12px_rgba(245,158,11,0.8)] animate-pulse"></div>
                           </div>
                       </div>
                       <div className="relative z-10 flex flex-col gap-2">
                           <span className="text-[10px] text-amber-500/90 font-mono uppercase tracking-[0.25em] animate-pulse font-bold">
                             正在显影
                           </span>
                       ...
                       <div className="absolute bottom-0 left-0 w-full h-px bg-gradient-to-r from-transparent via-amber-500/20 to-transparent opacity-50 animate-pulse"></div>
                    </>
                  )}
                  ...
                  {frame.status === 'pending' && (
                    <div className="opacity-20 flex flex-col items-center">
                        ...
                        <span className="text-[10px] text-zinc-500 font-mono uppercase tracking-widest">待处理</span>
                    </div>
                  )}
                </div>
              )}
               ...
               <div className="flex flex-col gap-2.5 px-1">
                  ...
                  <div className="pt-2 flex items-center justify-between border-t border-white/5 mt-1">
                     <span className="text-[10px] text-zinc-600 font-mono tracking-wider uppercase">
                       {frame.metadata?.model.includes('pro') ? '电影级' : '预览级'}
                     </span>
      ...
      {selectedFrame && selectedIndex !== null && (
        ...
        >
          {/* Top Bar */}
              <div className="flex-none h-16 flex items-center justify-between px-6 z-50 border-b border-white/5 bg-black/40">
                ...
                >
                    ...
                    <span className="hidden sm:inline">{copiedId === selectedFrame.id ? "已复制完整元数据" : "复制生成完整配方"}</span>
                </button>
             </div>
          ...
          {/* Bottom Bar */}
          <div className="flex-none p-6 border-t border-white/10 bg-[#0c0c0e] z-40" onClick={(e) => e.stopPropagation()}>
